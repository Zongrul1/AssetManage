...
<script>
  ...
  export default {
    ...
    created () {
        axios.get('http://localhost:9000/fund/get-all')
        .then((response) => {
        ...
        axios.get('http://localhost:9000/fund/get-value')        
        .then((response) => {
    ...
    methods: {
      ...
      handleDelete(id) {
        console.log(id)
        var url = "http://localhost:9000/fund/delete/"
        axios.get(url + id)
      ...
      update(){
        ...
        request({
          url:'http://localhost:9000/fund/update',
          method:'post',
      ...
      insert(){
        ...
        request({
          url:'http://localhost:9000/fund/add',
          method:'post',
          data:this.updateData//注意这里不需要转成json字符串，axios会自动识别该数据是form表单数据还是对象数据
      });
        window.location.reload()

...
<div>
  ...
  <el-col :span="5"><div class="grid-content bg-purple">
      <el-row>
        <el-col :span="12"><div class="total-value">数字货币总价值：</div></el-col>
...
<script>
  ...
  export default {
    ...
    created () {
        axios.get('http://localhost:10088/coin/get-all')
        .then((response) => {
        ...
        axios.get('http://localhost:10088/coin/get-value')        
        .then((response) => {
    ...
    methods: {
      ...
      handleDelete(id) {
        console.log(id)
        var url = "http://localhost:10088/coin/delete/"
        axios.get(url + id)
      ...
      update(){
        ...
        request({
          url:'http://localhost:10088/coin/update',
          method:'post',
      ...
      insert(){
        ...
        request({
          url:'http://localhost:10088/coin/add',
          method:'post',
          data:this.updateData//注意这里不需要转成json字符串，axios会自动识别该数据是form表单数据还是对象数据
      });
        window.location.reload()
